...
(function contactProfilePage() {
  ...
  async function load() {
    ...
    if (!window.CoSoApi) return;

    try {
      if (userId && window.CoSoApi.layNguoiDungTheoId) {
        fill(await window.CoSoApi.layNguoiDungTheoId(userId));
        return;
      }
      if (userKey && window.CoSoApi.layDanhSachNguoiDung) {
